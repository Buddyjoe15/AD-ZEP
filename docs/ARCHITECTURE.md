...
Terrain is regenerated from the seed by the save's map type (`map`: `grass` for new games, `forest` for saves from before v0.7, `woodlands` for the generated Woodlands in `src/world/woodlands.js`; see `GW.MapGen.types`), and `terrainEdits` replays any changes made after generation, including Map Editor strokes. Generators also take the landing site (`landing`, tiles; `GW.MapGen.landing()` keeps it inside the map), which the save records because Woodlands builds its landing clearing there; a generator may move it (Woodlands keeps it off water), and `State.landing` is where it ended up. The forest and Woodlands generators must keep their RNG call order: terrain fingerprint tests fail if generation changes. Woodlands also builds `grid.art` (height levels, waterfall and fallen-tree directions, ground detail), which only the renderer (`src/render/woodlands.js`) reads; it is rebuilt with the terrain and never saved. To add a map type, add a generator to `GW.MapGen.types`; never change what an existing type generates.
