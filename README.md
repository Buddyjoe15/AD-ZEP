...
- **Choose where the ship crash-lands.** New Game now has a **Map** choice (test map or Woodlands) and a **Landing site**: centre of the map, random, or a tile you type in (64–447). In a game, Map Editor → Maps → **Preview** has **Choose landing site**: tap the full map and the preview is generated again around that spot, then **Start a new expedition here** (replaces the game in this slot) or **Load terrain into this game** (keeps your game; the clearing goes at that spot). Woodlands keeps the landing clearing off water: a site within 36 tiles of a river, lake or creek moves to the nearest dry ground, and the game says where. Rivers, the lake and the creek never move for it; villages, zones, streams and trails are placed around it. A landing at the centre generates exactly as before.
- **Save format: schema 8.** Saves record the landing site (`landing`, in tiles), because the map generator builds the landing clearing there. `migrate_7_to_8` gives older saves the centre of their world, which is where every earlier ship landed.
...
- **Crowds no longer jam against walls.** A unit pushed by the crowd against the edge of a wall or building could get stuck for good: it could not be pushed along the wall, and a unit whose next waypoint ended up straight through the wall kept walking into it. Pushed units now slide along walls, and a unit heading almost straight into one drops its route and re-plans. This also fixes the Hostile Fabricator sometimes stopping short of its count, when a unit stuck beside the Shield Projector kept its spawn point blocked; as a safety net, the Fabricator also re-routes any held unit on its spawn point whose next waypoint is no longer in a clear line.
- **Woodlands pixel art redrawn at 48 × 48.** To match the rest of the pixel art (now 1 art px per world px), the Woodlands grass, tall grass, water, shorelines, cliffs and all three tree kinds are drawn at 48 × 48 per tile with more detail: two-octave grass with short blades, clover, tufts and the odd flower; taller grass blades and seed heads; ripples with crests and troughs; wider foamy banks; cliff faces with grained, chipped stone, lit cracks and moss; oaks with leaf clumps, pines with needles along each spoke, birches with shaded clusters and pale twigs. Trees lean 2 and 4 art px (the same distance on screen as before) and their shadow sits at 4, 4. Close-up terrain uses the new double-resolution chunks, and the lower-resolution chunks at middle zoom stay. Saves are unchanged.
